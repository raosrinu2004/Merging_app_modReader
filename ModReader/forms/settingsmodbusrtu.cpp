...
void SettingsModbusRTU::tachusChanges(QString port)
{
    if (m_settings != NULL) {

        m_settings->setSerialPort(port, ui->cmbDev->currentText());
        m_settings->setBaud("19200");
        m_settings->setDataBits("8");
        m_settings->setStopBits("1");
        m_settings->setParity("Even");
        m_settings->setRTS("Disable");
    }
}
